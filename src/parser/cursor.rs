...
impl Cursor {
  ...
  pub fn commit(&mut self) {
    let line = Line {
      ...
      indent: if self.indent.is_empty() { None } else { Some(self.indent.clone()) },
      comment: if self.comment.is_empty() { None } else { Some(self.comment.clone()) },
      variable: if self.variable.is_empty() { None } else { Some(self.variable.clone()) },
      margin: if self.margin.is_empty() { None } else { Some(self.margin.clone()) },
    };
